const dt = {
  ...
  parseValueString( valStr, type) {
    ...
    if (dt.isStringType( type)) {
      for (const str of valueStringsToParse) {
        if (!dt.ranges[type].condition( str)) {
      ...
      for (const str of valueStringsToParse) {
        if (isNaN( parseInt( str)) || !dt.ranges[type].stringCondition( str)) {
          value = undefined; break;
        } else {
      ...
      for (const str of valueStringsToParse) {
        if (isNaN( parseFloat( str)) || !dt.ranges[type].stringCondition( str)) {
          value = undefined; break;
        } else {
      ...
      default:
        ...
        } else if (type in dt.ranges && "str2val" in dt.ranges[type] &&
            dt.isOfType( value, type)) {
          value = dt.ranges[type].str2val( valStr);
        } else {
          value = undefined;
        }
      }
    ...
    if (dt.isStringType( type)) {
      ...
      switch (type) {
        ...
        case "Record":
          ...
          break;
        default:
          if (type in dt.ranges && "val2str" in dt.ranges[type] &&
              dt.isOfType( value, type)) {
            string = dt.ranges[type].val2str( value);
          }
      }
    }
    return string;
  ...
  isOfType( value, Type) {
    const cond = dt.ranges[Type]?.condition;
  ...
  check( attr, decl, val, optParams) {
     ...
     ********************************************************************/
    if (range in dt.ranges) {
      for (const v of valuesToCheck) {
        if (!dt.ranges[range].condition( v)) {
          constrVio.push( new RangeConstraintViolation(
              `The value ${v} of attribute ${attr} is not ${dt.ranges[range].phrase}!`));
        }
      }
    } else {
