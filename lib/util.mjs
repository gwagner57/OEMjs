...
util.class2TableName = function (className) {
  ...
  if (className.charAt(0) === className.charAt(0).toUpperCase()) { // starts with upper case
    ...
    if (lowercaseClassName in util.specialPluralForms) {
      classNamePluralForm = util.specialPluralForms[lowercaseClassName];
    } else if (className.charAt( className.length-1) === "y") {
      classNamePluralForm = className.slice( 0, className.length-1) + "ies";
    } else {
      classNamePluralForm = className + "s";
    }
    tableName = util.camelToLowerCase( classNamePluralForm);
    ...
    tableName = util.invCamelToUppercase( classNamePluralForm);
  }
  return tableName;
};
// Example: books => Book
